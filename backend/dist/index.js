...
app.post("/api/v1/content", middleware_1.userMiddleware, async (req, res) => {
    ...
    try {
        const { tags, vector } = await (0, semantic_1.deriveTagsForContent)({
            userId: req.userId,
            title,
            link,
            type,
        });
        const createdContent = await db_1.prisma.content.create({
            data: {
                link,
                type,
                title,
        ...
        if (vector) {
            void (0, semantic_1.upsertContentEmbedding)({
...
app.get("/api/v1/content", middleware_1.userMiddleware, async (req, res) => {
    ...
    const tagQuery = typeof req.query.tag === "string" && req.query.tag.trim().length > 0
        ? req.query.tag.trim().toLowerCase()
        : undefined;
    const typeQuery = typeof req.query.type === "string" &&
...
app.get("/api/v1/tags", middleware_1.userMiddleware, async (req, res) => {
    if (!req.userId) {
        return res.status(403).json({
            message: "You are not logged in",
        });
    ...
    try {
        const contentWithTags = await db_1.prisma.content.findMany({
            where: {
                userId: req.userId,
            },
            select: {
...
app.delete("/api/v1/content", middleware_1.userMiddleware, async (req, res) => {
    ...
    try {
        const deleted = await db_1.prisma.content.deleteMany({
            where: {
                id: contentId,
                userId: req.userId,
            },
        ...
        if (deleted.count > 0) {
            void (0, semantic_1.deleteContentEmbedding)(contentId);
        }
        return res.json({
            message: "Deleted",
        });
